...
object TestDBs {
  def H2Mem(cname: String) = new JdbcTestDB("h2mem", H2Driver) {
  ...
  def H2Disk(cname: String) = new JdbcTestDB("h2disk", H2Driver) {
    val dbName = "h2-"+cname
  ...
  def Postgres(cname: String) = new ExternalJdbcTestDB("postgres", PostgresDriver) {
  ...
  def MySQL(cname: String) = new ExternalJdbcTestDB ("mysql", MySQLDriver) {
    // Recreating the DB is faster than dropping everything individually
    override def dropUserArtifacts(implicit session: profile.Backend#Session) = {
      session.close()
      cleanUpBefore()
...
class SQLiteTestDB(dburl: String, confName: String) extends JdbcTestDB(confName, SQLiteDriver) {
  val url = dburl
...
class AccessDB(confName: String) extends JdbcTestDB(confName, AccessDriver) {
...
abstract class DerbyDB(confName: String) extends JdbcTestDB(confName, DerbyDriver) {
...
abstract class HsqlDB(confName: String) extends JdbcTestDB(confName, HsqldbDriver) {
...
class SQLServerDB(confName: String) extends ExternalJdbcTestDB (confName, SQLServerDriver) {
