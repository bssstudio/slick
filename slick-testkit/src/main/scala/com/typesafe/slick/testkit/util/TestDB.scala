...
import scala.slick.profile.{BasicDriver, BasicProfile, Capability}
...
abstract class TestDB(final val confName: String) {
 ...
 */
abstract class JdbcTestDB(confName: String, final val driver: JdbcDriver) extends TestDB(confName) {
  val profile: JdbcProfile = driver
...
class ExternalJdbcTestDB(confName: String, driver: JdbcDriver) extends JdbcTestDB(confName, driver) {
